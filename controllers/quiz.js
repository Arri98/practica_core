...
exports.load = (req, res, next, quizId) => {
    ...
    .then(quiz => {
        if (quiz) {
         var promises=[];
            var tips=[];
            quiz.tips.forEach((tip,index)=>{
               promises.push(
                models.tip.findByPk(tip.id,{
                include:[
                    {model: models.user, as:'authorTip'}
                ]}
                )
                .then(tiptemp => {
                    if (tiptemp) {
...
exports.randPlay = (req, res, next) =>{
    ...
    const query={
        ...
        where:{
           id:{ [Op.notIn]: req.session.randomPlay}
         }
     };
...
exports.randCheck= (req, res, next) =>{
